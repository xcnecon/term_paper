...
The null hypothesis is that a variable does not guarantee cause profit. Except for investment, we can reject the null hypotheses at a 5% significance value. And for the variables combined together, we can safely conclude that they will cause next-quarter profit. 
...
- tradeoff: too few lags might produce volatile predictions; too many lags require a long lookback period, which reduces the series of predictions.
- Multiple combinations of a number of lags and lookback periods are tested, and all of them are reported. No iteration or data mining.
- Results of models using level changes are documented but not reported since their performance is very poor. 

## Results
- x-y means x lags and y lookback period
- control models only use past profit and data, which captures the self-correlation of profit and potential trends. 
- Models of any set of parameters can significantly beat their control models. Performance increases with the number of lags and lookback period. 
